...
- 自己绝了领出花色时（`voidFillCards`），根据 `partnerAtRisk` 判断：
  - **`partnerAtRisk = true`**（该花色还有更大的未出牌，队友可能被后续对手压走）：
    - 用主将吃，不垫分
    - 若已知后手对手也绝该花色（`enemySubsequentVoidInLead`）且本墩已有分：直接用最大主牌抢住，防止后手再截胡拿分
    - 若已知后手对手也绝该花色但本墩暂无分：用安全出主策略（`isSafeTrumpFiller`，A/王/级牌），避免无谓交最大主
    - 若后手对手仍有该花色：优先出孤张主牌分（10/K/5，不拆对子）将吃；无主牌分则出最弱主牌
    - 无主牌时：安全垫牌（不送分）
